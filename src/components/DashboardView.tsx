...
function DynamicDashboardView({ project }: { project: ProjectStatusResponse }) {
  ...
  useEffect(() => {
    // 1. Simulate minor visitor fluctuations
    ...
    // 2. Simulate rapid ingestion event counters
    const eventsInterval = setInterval(() => {
      setIngestedEvents((prev) => prev + Math.floor(Math.random() * 12) + 1);
    }, 800);

    // 3. Simulate cluster latency jitter
    ...
    const logInterval = setInterval(() => {
      ...
      setLogs((prevLogs) => {
        ...
        return updated.slice(-5); // Keeps logs clean in viewport
      });
    }, 5000);
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-100 flex flex-col justify-between font-sans selection:bg-cyan-500 selection:text-black overflow-x-hidden relative">
      {/* Background ambient orbs */}
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-gradient-to-br from-blue-950/25 to-purple-950/10 blur-[120px] rounded-full pointer-events-none -z-10" />
      <div className="absolute top-1/3 right-10 w-[400px] h-[400px] bg-gradient-to-br from-cyan-950/20 to-transparent blur-[100px] rounded-full pointer-events-none -z-10" />

      {/* Header Bar */}
      <header className="border-b border-slate-900 bg-[#030712]/40 backdrop-blur-md px-6 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {project.profile?.logoUrl ? (
              <div className="relative h-10 w-10 overflow-hidden rounded-lg border border-slate-800 bg-white/5 p-1">
                <Image
              ...
              <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center border border-blue-500/30 shadow-[0_0_15px_rgba(37,99,235,0.4)]">
          ...
          <div className="flex items-center gap-2 bg-slate-900/50 border border-cyan-500/20 rounded-full px-3 py-1.5 backdrop-blur-sm">
      ...
      <main className="max-w-7xl mx-auto w-full px-6 py-12 flex-grow grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="inline-flex items-center gap-2 border border-purple-900/50 bg-purple-950/20 rounded-full px-4 py-1">
          ...
          <div className="space-y-2">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-white leading-tight">
              The Intelligence
              <span className="block mt-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent filter drop-shadow-[0_0_30px_rgba(6,182,212,0.15)]">
                Telemetry Pipeline.
              </span>
            </h2>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <button className="flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black font-semibold text-xs py-3 px-5 rounded-lg transition-all duration-200 shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_25px_rgba(6,182,212,0.5)] transform active:scale-95">
              Access Master Portal
              <ArrowRight className="h-4 w-4" />
            </button>
            <button className="flex items-center gap-2 bg-slate-900/80 hover:bg-slate-800 border border-slate-800 text-slate-300 font-semibold text-xs py-3 px-5 rounded-lg transition-all duration-200">
              Explore API Docs
        ...
        <div className="lg:col-span-7 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-[#030712]/40 border border-slate-900 rounded-xl p-5 backdrop-blur-sm space-y-3 shadow-md">
          ...
          <div className="bg-[#030712]/40 border border-slate-900 rounded-xl p-5 backdrop-blur-sm space-y-3 shadow-md">
            <div className="flex items-center justify-between text-[#3bf7a0]">
              <span className="text-[9px] font-bold tracking-widest uppercase font-mono text-slate-400">
                Waf Threat Shield
              </span>
          ...
          <div className="md:col-span-2 bg-[#030712]/60 border border-slate-900 rounded-xl p-5 backdrop-blur-sm space-y-4 shadow-md flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Terminal className="h-4 w-4 text-cyan-400" />
                <h3 className="text-[10px] font-bold tracking-widest text-slate-300 uppercase font-mono">
            ...
            <div className="bg-[#030712] border border-slate-900 rounded-lg p-4 font-mono text-[10px] leading-relaxed text-slate-300 space-y-2 h-[155px] overflow-y-auto scrollbar-none">
              {logs.map((log, index) => (
                <div
                  key={index}
          ...
          <div className="bg-[#030712]/40 border border-slate-900 rounded-xl p-5 backdrop-blur-sm space-y-4 shadow-md flex flex-col justify-between">
            ...
            <div className="space-y-3 flex-grow py-1">
              {clusters.map((cluster) => (
                <div
                  key={cluster.name}
                  className="flex items-center justify-between border-b border-slate-900/50 pb-2 last:border-0 last:pb-0"
                >
                  <div className="space-y-0.5">
                  ...
                  <div className="text-right space-y-0.5">
                    ...
                    <span className="text-[8px] font-mono text-[#3bf7a0] uppercase flex items-center justify-end gap-1 font-semibold">
                      <span className="w-1 h-1 rounded-full bg-[#3bf7a0] animate-pulse"></span>
                      {cluster.status}
                    </span>
                  </div>
                </div>
            ...
            <p className="text-[8px] leading-normal text-slate-500 font-mono mt-1">
      ...
      {/* Footer bar */}
      <footer className="border-t border-slate-900 bg-[#030712]/20 px-6 py-6 z-10">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          {project.profile?.logoUrl ? (
            ...
            <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 font-mono text-3xl font-black text-white shadow-xl shadow-cyan-500/10">
              {project.name.charAt(0).toUpperCase()}
            </div>
          )}
    ...
    <div className="min-h-screen w-screen bg-[#030712] flex items-center justify-center p-6 text-white relative overflow-hidden">
      ...
      <div className="max-w-md w-full text-center space-y-6 relative z-10">
        ...
        <a
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-2.5 text-sm font-bold text-black shadow-lg hover:opacity-90 transition-all"
        >
